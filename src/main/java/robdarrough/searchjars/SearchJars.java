 ...
 * 
 * Very useful for resolving compilation issues involving a missing class file
 * or a conflict between class files. The compilers messages will only name the
 * class files. This utility identifies the corresponding jar files. It is also
 * good for finding corrupted archives.
 */
public class SearchJars {
	...
	private FileFilter archiveFilesFilter = new FileFilter() {
		...
		public boolean accept(File f) {
			return f.getName().toUpperCase().endsWith(".JAR") || f.getName().toUpperCase().endsWith(".ZIP")
					|| f.getName().toUpperCase().endsWith(".WAR") || f.getName().toUpperCase().endsWith(".EAR");
		}
	};
